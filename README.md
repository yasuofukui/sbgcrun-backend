...
書籍「Cloud Run本」のバックエンドAPI用のダウンロードリポジトリです。
...
export DB_USERNAME=sbgcrunapp
export DB_PASSWORD=password
export DB_NAME=sbgcrunapp
export DB_CONN=1
```
...
```bash
$ docker build -t sbgcrun-backend:latest .
$ docker images
REPOSITORY                  TAG                 IMAGE ID            CREATED             SIZE
sbgcrun-backend                   latest              cdb20b70f267        58 minutes ago      4.45MB
:
$ docker run -d -p 80:80 sbgcrun-backend:latest
```
